 ...
 * @public
 */
export class TopLevelAPI extends BaseAPI {
  /**
  ...
  postTopLevel({ treeList }: { treeList: Array<Tree> }): Promise<unknown> {
    return this.request({ method: 'post', data: treeList, path: '/TopLevel' })
  }
  ...
  getTopLevelById({
    ...
    return this.request({
      method: 'get',
      path: `/TopLevel/${id}`,
      params: { include_agents, include_teams, includedetails },
    })
  }
   ...
   * @param {number} id
   */
  deleteTopLevelById({ id }: { id: number }): Promise<unknown> {
    return this.request({ method: 'delete', path: `/TopLevel/${id}` })
  }
}
