...
export class FAQListsAPI extends HaloPSA {
   ...
   */
  postFAQLists(fAQListHead: Array<FAQListHead>): Promise<any> {
    return this.request({
      method: 'post',
      path: '/FAQLists',
      data: fAQListHead,
   ...
   * @param {number} organisation_id 
   */
  getFAQListsById(id: number, includedetails: boolean, organisation_id: number): Promise<any> {
    return this.request({
      method: 'get',
      path: `/FAQLists/${id}`,
      params: {
   ...
   * @param {number} id
   */
  deleteFAQListsById(id: number): Promise<any> {
    return this.request({
      method: 'delete',
      path: `/FAQLists/${id}`,
    })
