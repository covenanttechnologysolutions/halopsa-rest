...
export class ClientAPI extends HaloPSA {
   ...
   */
  postClient({ area }: { area: Array<Area> }): Promise<any> {
    return this.request({
      method: 'post',
      path: '/Client',
      data: area,
   ...
   * @param {number} id
   */
  deleteClientById({ id }: { id: number }): Promise<any> {
    return this.request({
      method: 'delete',
      path: `/Client/${id}`,
    })
   ...
   */
  postClientNewAccountsId({ area }: { area: Array<Area> }): Promise<any> {
    return this.request({
      method: 'post',
   ...
   */
  postClientPaymentMethodUpdate({ area }: { area: Array<Area> }): Promise<any> {
    return this.request({
      method: 'post',
