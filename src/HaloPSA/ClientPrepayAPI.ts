...
export class ClientPrepayAPI extends HaloPSA {
   ...
   */
  postClientPrepay(prepayHistory: Array<PrepayHistory>): Promise<any> {
    return this.request({
      method: 'post',
      path: '/ClientPrepay',
      data: prepayHistory,
   ...
   */
  getClientPrepayById(id: number, includedetails: boolean): Promise<any> {
    return this.request({
      method: 'get',
   ...
   * @param {number} id
   */
  deleteClientPrepayById(id: number): Promise<any> {
    return this.request({
      method: 'delete',
      path: `/ClientPrepay/${id}`,
    })
