...
export class ProjectsAPI extends BaseAPI {
  ...
  postProjects({ faultsList }: { faultsList: Array<Faults> }): Promise<Faults> {
    return this.request({ method: 'post', data: faultsList, path: '/Projects' })
  }
   ...
   * @param {string} [reason]
   */
  deleteProjectsById({ id, reason }: { id: number; reason?: string }): Promise<unknown> {
    return this.request({ method: 'delete', path: `/Projects/${id}`, params: { reason } })
  }

  postProjectsView({ faultsList }: { faultsList: Array<Faults> }): Promise<unknown> {
    return this.request({ method: 'post', data: faultsList, path: '/Projects/View' })
  }
}
