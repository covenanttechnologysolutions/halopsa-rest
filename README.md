...
```sh
npm install halopsa-rest
```

## Quick start

```ts
import { HaloAPI } from 'halopsa-rest'
...
```ts
import type { Faults, Faults_View } from 'halopsa-rest/dist/HaloPSA/TicketsAPI'
...
```ts
import type { components } from 'halopsa-rest/dist/types'
...
- `halopsa-rest.HaloPSATypes` is large (~1.4 MB on disk). Treat it as a type-only import; tree-shaking won't help if you accidentally `require()` it at runtime.
